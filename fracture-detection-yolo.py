...
def detect_fracture(image_path):
    ...
    for result in results:
        ...
        for i, box in enumerate(boxes):
            ...
            print(f"Detecção {i}: {x1}, {y1}, {x2}, {y2}")
        if len(boxes) > 0:
            detected = True
            show_image_with_boxes(temp_path, result)
...
def show_image_with_boxes(image_path, result):
    image = cv2.imread(image_path)

    for box in result.boxes.xyxy:
        x1, y1, x2, y2 = map(int, box)
        cv2.rectangle(image, (x1, y1), (x2, y2), (0, 255, 0), 3)  # Caixa verde
...
detect_fracture("raiox/raiox1.jpg")
